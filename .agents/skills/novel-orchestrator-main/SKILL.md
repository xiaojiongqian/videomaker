...
- 这个张力可以来自：
  ...
  - 结果已经快要落下的瞬间
  - 情感冲击或不合常理的局面
- 不要用缓慢回顾、背景交代、平直说明去占掉开头最有力量的空间

对纪实骨架写法，这不意味着必须虚构戏剧性，而是要优先切进：
...
如果一章开头读完后，读者仍然感觉“还没真正开始”，应视为失败并回退重写。
...
- 本章最重要的场面里，谁当下最想拿到什么，什么在拦他，哪次动作改变了局面，代价最后落到了哪里

如果这条因果链说不清，章节通常仍然只是“事情发生了”，还不是“事情为什么这样发生”。
...
- 多个比赛或事件明明提供的是同一种叙事功能，却被按相同厚度逐个罗列
- 每个节点都按“时间 -> 比分 / 结果 -> 一次动作 -> 一句意义”同模版展开，导致读感被平均
- 本来应该形成节奏反差的内容，被写成稳定重复的资料带

对比赛线、成长线、纪实线，默认执行这条压缩规则：

- 如果 3 个以上节点主要在回答同一个问题，只选最有压力、最有偏转、最能代表代价的一到两个节点写成硬场面
- 其余同型节点优先打包成 `bridge cluster`
  - 只交代必要结果、累计变化和为什么它们会把压力推到下一个场面
  - 对桥接段，优先使用“一件事一短段”的写法；不要让 bridge cluster 自己长成正常叙述段
- 不要把“都很重要”误写成“都要写一样多”
- 章节节奏应当故意出现快慢、粗细和轻重差，而不是平均配给
- 默认假设每章只有 `1` 个 dominant scene，最多 `2` 个 expanded scenes；超过这个数量时，应先证明每个节点都在改变不同的 ledger
- 最重要的场面应明显慢于周围段落：允许连续两到三段只服务这一个节点
- 次要节点如果不能承受这种慢速展开，就不要硬写成完整段，应退回桥接

对 dominant scene，再额外执行 `slow the hinge moment`：

- 关键动作前后允许故意放慢
- 把一个决定性瞬间拆成进入、动作、结果、反应
- 让读者不只知道“发生了什么”，还知道“这一下是怎么逼出来的”
- 关键时刻可以多给两三个具体细节，但这些细节必须服务阻力、选择或代价

如果用户明确要求“桥接更短”“别再像流水账”“一件事一段”，默认启用 `micro-bridge cuts`：
...
- 一段只搬一件事，通常只保留：
  ...
  - 一个累计变化
- 允许连续出现多个极短段，形成快切
- 这些短段的作用是把读者迅速推到下一个硬场面，不是自己变成新的展开段
- 如果 bridge 段开始出现完整解释、完整转折和完整抒发，说明它已经过长，应继续压缩
- 但 `micro-bridge` 不能碎成词条式断片；一个完整语义单元应至少落成一个完整短段
- 紧密绑定的元素应放在同一段里，例如：对手 / 关键人 / 比分，或 时间 / 结果 / 累计变化
- 比分不要默认拆成独立悬空短段；如果它和对手、关键动作、结果属于同一语义链，优先留在同一短段里，只有在比分本身就是那一段唯一的偏转点时才单独站出来
...
对关键硬节点，再追加 `dramatic causality pass`：
...
如果一段只有结果，没有“谁做了什么把结果推成这样”，它通常还不够立体。

再补一条 `show-not-tell pass`：

- 能用动作、身体反应、空间关系、物件变化表现的，不优先用抽象判断直接说
- 能把“愤怒、紧张、被压住、被逼到前面”落成可见局面的，不优先用概念词总结
- 但也不要为了“展示”去堆空洞景物；展示必须服务当前冲突
...
5. 检查正文表面是否泄漏作者工作台
   ...
   - 同时检查是否把多个同型比赛节点写成了同一种段落模板；如果有，必须回退并重做轻重分配
...
16. 是否把多个同型节点用几乎相同的句法、相同的段长和相同的解释力度逐个铺开
17. 如果删掉其中一半比赛段，本章是否几乎不损失读感；若是，说明还没有做真正的轻重分配
18. 最值得展开的那一个场面，是否真的比周围桥接段更慢、更具体、更可复述
19. 主场面是否至少占据全章最有分量的连续段落，而不是和桥接段平均分篇幅
20. 读者是否能说清主场面的即时目标、阻力、主动动作、偏转点和代价；若不能，说明因果链仍未落地
21. 文中的矛盾是否主要靠作者概括，还是已经落实成“想赢 / 赢不了”“想推进 / 被堵住”“先罚进 / 仍然无能为力”这类可见局面
...
- 多个同型比赛节点被用几乎相同的展开模板逐段罗列，导致章节节奏发平
